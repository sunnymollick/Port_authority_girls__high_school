...
@if(count($data) > 0)
    ...
    @else
        <form id='special_create' action="" enctype="multipart/form-data" method="post" accept-charset="utf-8">
            <input type="hidden" name="exam_id" value="{{ $data[0]->exam_id }}">
            <input type="hidden" name="class_id" value="{{ $data[0]->class_id }}">
            <input type="hidden" name="section_id" value="{{ $data[0]->section_id }}">
            <input type="hidden" name="subject_id" value="{{ $data[0]->sub_id }} ">
            ...
            <div class="col-md-12 col-sm-12 table-responsive">
                <table id="manage_all" class="table table-collapse table-bordered table-hover">
                    ...
                    <tr>
                        ...
                        <th class="std_name">Name</th>
                        <th class="text-center">Roll</th>
                        <th class="marks">Written Marks</th>
                        <th class="marks">Assignment Marks</th>
                        <th class="marks">Other Marks</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach($data as $row)
                        <tr>
                            ...
                            <td class="text-center">{{ $row->std_roll }}</td>
                            <td><input type="number" max="50" min="0" class="form-control decimal"
                                    name="written_{{$row->std_code}}"
                                    value="{{$row->written_marks ? $row->written_marks : ''}}"></td>
                            <td><input type="number" max="40" min="0" class="form-control decimal"
                                    name="assignment_{{$row->std_code}}"
                                    value="{{$row->assignment_marks ? $row->assignment_marks : ''}}"></td>
                            <td><input type="number" max="50" min="0" class="form-control decimal"
                                    name="other_{{$row->std_code}}"
                                    value="{{$row->other_marks ? $row->other_marks : ''}}"></td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
...
<script type="text/javascript">
    ...
    $('#special_create').validate({// <- attach '.validate()' to your form
        // Rules for form validation
        rules: {
            name: {
                required: true
        ...
        submitHandler: function (form) {
            swal({
                ...
                var myData = new FormData($("#special_create")[0]);
                var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
                myData.append('_token', CSRF_TOKEN);
