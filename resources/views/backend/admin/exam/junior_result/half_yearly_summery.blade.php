@if(!empty($data))
    ...
    // dd($data['result']);
                    ...
                    <tr>
                        ...
                        @if ((int) $row->class_rank < 10)
                                <td class="text-center"><span>&nbsp;&nbsp;</span>{{ $row->class_rank }}</td>
                        @elseif((int) $row->class_rank < 100)
                                <td class="text-center"><span>&nbsp;</span>{{ $row->class_rank }}</td>
                        @else
                                <td class="text-center">{{ $row->class_rank }}</td>
                        @endif

                        @if ((int) $row->rank < 10)
                                <td class="text-center"><span>&nbsp;&nbsp;</span>{{ $row->rank }}</td>
                        @elseif((int) $row->rank < 100)
                                <td class="text-center"><span>&nbsp;</span>{{ $row->rank }}</td>
                        @else
                        ...
                        <td class="text-center"><a data-toggle='tooltip' title='View Tabulation Sheet'
                                                ...
                                                <a data-toggle='tooltip' title='Print Tabulation Sheet' style="margin-left: 2px;"
                                                class="btn btn-info print" std_roll="{{ $row->stdRoll }}"
                                                std_name="{{ $row->stdName }}"
                                                std_code="{{ $row->stdCode }}"
                                                rank="{{ $row->rank }}"
                                                class_rank="{{ $row->class_rank }}"
            ...
            <table id="manage_all" class="table table-collapse table-bordered table-hover">
                ...
                @foreach($data['result'] as $row)
                    ...
                    <tr>
                        ...
                        @endif
                        <td class="text-center"><a data-toggle='tooltip' title='View Tabulation Sheet'
                                                class="btn btn-info view" std_roll="{{ $row->stdRoll }}"
                                                std_name="{{ $row->stdName }}"
                                                std_code="{{ $row->stdCode }}"
                                                rank="{{ $row->rank }}"> View </a><a data-toggle='tooltip' title='Print Tabulation Sheet' style="margin-left: 2px;"
                                                class="btn btn-success print" std_roll="{{ $row->stdRoll }}"
                                                std_name="{{ $row->stdName }}"
                                                std_code="{{ $row->stdCode }}"
                                                rank="{{ $row->rank }}"> Print </a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
...
<script type="text/javascript">
    ...
    $("#manage_all").on("click", ".view", function () {
        ...
        var class_rank = $(this).attr('class_rank');
    ...
    $("#manage_all").on("click", ".print", function () {
        ...
        var base = '{!! route('admin.jrHalfyearlyMarksheetPrint') !!}';
        var url = base + '?class_id=' + class_id + '&section_id=' + section_id + '&exam_id=' + exam_id + '&exam_name=' + exam_name + '&section_name=' + section_name + '&class_name=' + class_name + '&std_code=' + std_code + '&std_name=' + std_name + '&std_roll=' + std_roll + '&rank=' + rank+ '&class_rank=' + class_rank;
        window.open(url);
    });
</script>
<script>
